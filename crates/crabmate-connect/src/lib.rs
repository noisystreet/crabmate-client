//! 桌面 / 移动壳共用的「连接远程 `crabmate serve`」逻辑。
//!
...
//!
//! 默认 **不**依赖 Tauri（probe / handoff / keyring 可单独测试）。壳启用 feature `tauri`
//! 后才编译 invoke 命令与 WebView 导航钩子。
//!
...
pub use connect_home::{SuggestedServerUrl, seed_connect_home};
pub use handoff::{
...
pub use navigation::{
    ShellNavigationDecision, decide_shell_navigation, is_app_origin, is_connect_page_url,
};
pub use probe::{
