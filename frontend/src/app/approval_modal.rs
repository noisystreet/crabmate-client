        ...
        <Show when=move || pending_approval.get().is_some()>
            <div class="modal-backdrop approval-modal-backdrop">
                <FocusableModalPanel
                    ...
                    on_escape=Callback::new(move |_| deny_pending_approval(signals))
                >
...
mod approval_modal_source_tests {
    #[test]
    fn approval_modal_uses_shared_dialog_shell_and_esc_denies() {
        let src = include_str!("approval_modal.rs");
        assert!(src.contains("FocusableModalPanel"));
        assert!(src.contains("deny_pending_approval"));
        assert!(src.contains("labelledby=\"approval-modal-title\""));
        assert!(src.contains("testid=\"approval-modal\""));
    }
}
