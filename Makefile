# crabmate-client 构建入口：桌面 / Android 壳、质检与清理。
# 用法：make help
# 壳不 spawn serve；业务 UI 可选经 CRABMATE_FRONTEND_DIST / 同级主仓 frontend/dist 同步。
...
# 1=构建主仓/指定 frontend（需 CRABMATE_FRONTEND_DIR 或同级主仓）
CM_MOBILE_BUILD_FRONTEND ?= 0

# 可选：打 desktop deb 时同步进 dist 的 UI 产物目录
...
.PHONY: help all \
	...
	clean clean-desktop clean-mobile clean-connect

help:
	@echo "crabmate-client Makefile（仓库根目录执行）"
	@echo ""
	@echo "构建："
	@echo "  make prepare-sidecar     同步 connect/splash（及可选 frontend dist）到 desktop-tauri/dist"
	...
	@echo ""
	@echo "质检："
	@echo "  make check               bash scripts/check.sh（fmt/clippy/复杂度）"
	@echo "  make test                connect + desktop cargo test（Victauri 默认跳过）"
	@echo "  make fmt                 三包 cargo fmt"
	@echo "  make clippy              三包 clippy -D warnings"
	@echo "  make victauri-e2e        全量 Victauri（需外部 crabmate serve）"
	@echo ""
	@echo "清理："
	@echo "  make clean               清理 desktop/mobile/connect 产物"
	...
	@echo ""
	@echo "变量：CRABMATE_FRONTEND_DIST=…（desktop prepare）"
	@echo "      MOBILE_ANDROID_TARGET=aarch64  CM_MOBILE_GRADLE_STOP=1"
	@echo "      CM_MOBILE_BUILD_FRONTEND=1（apk 时 trunk 构建 UI）"
