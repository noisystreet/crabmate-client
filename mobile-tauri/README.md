...
`gen/android/app/build.gradle.kts` 中 release 的 `usesCleartextTraffic=true` 与 `network_security_config.xml` 为**局域网**明文 HTTP WebView 而设（Android NSC 无法按 RFC1918 网段放行）。连接层（`crabmate-connect`）会拒绝公网 `http://` 主机，公网须 **HTTPS**；探测禁止跨 host 重定向。若重新执行 `tauri android init`，需再确认上述补丁、下方 **Android TLS 平台验证器**补丁与签名配置仍在。
