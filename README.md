...
| Contract crate | crates.io `crabmate` `0.5.0` + `protocol` (see [contract_pin.md](docs/design/contract_pin.md)) |
...
./crates/crabmate-tui/target/debug/crabmate-tui \
  --api-base http://127.0.0.1:8080 \
  repl
# In repl: /help · /status shows the model · /model switches model · /workspace [path] · /conv list|new|use <id> · Ctrl+C stops the turn (twice quits) · /resume re-attaches a dropped run
```
