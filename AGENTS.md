...
- CSS literal **budget gate**: in `frontend/styles/*.css` (excluding `tokens.css`, whose job is to hold literals) each file's count of `font-size` literals, of color literals (`#hex` / `rgb()` / `hsl()` / `white` / `black`), and of `border-radius` literals (`px` / `rem` / `em` / `%`; a bare `0` does not count, but a `var(--radius-x, 5px)` fallback does) must **equal** its per-file budget in `scripts/css_literals_budget.txt` — one more fails, and cleaning one up without lowering the budget fails too (the budget is ratcheted, not a floor); a stale entry (file gone / out of scope / all three counts 0) fails as well; enforced by `scripts/check-css-literals.sh` (hook + `scripts/check.sh`)
- Design-token **mirror gate**: color tokens have one source, `:root` in `frontend/styles/tokens.css`, but three surfaces cannot reference CSS variables and must restate the values — `mobile-tauri/src-tauri/gen/android/**/res/values/colors.xml` (the `cm_<token>` mirrors, one-to-one with `--<token>`, no other literal colors allowed), `res/values*/themes.xml` (no color literal; `Theme.crabmate_mobile` must reference `@color/cm_bg` for its four background slots and declare `android:windowLightStatusBar` / `android:windowLightNavigationBar` `= false` — the background is always the dark `--bg` token while `DayNight`'s light variant defaults to dark icons, invisible on a dark bar), and the boot backgrounds of `frontend/index.html` / `desktop-tauri/splash.html` / `crates/crabmate-connect/assets/connect.html` (must equal `--bg`) — plus two holes that would otherwise fail silently: every `@color/<name>` referenced from a `themes.xml` must actually be declared in `colors.xml` (a typo like `cm_textt` only surfaces at AAPT build time), and hand-written Kotlin under `.../java/edu/crabmate` (the `ktlint-android.sh` scope, generated sources excluded) may only consume `R.color.cm_*` — any other color resource bypasses the mirror chain, since its value is written by hand; all five rules are exact-equality with **no allowlist**, and each boot-background declaration must match exactly once so a rename/relocation fails instead of silently disabling the check; enforced by `scripts/check-token-mirrors.sh` (hook + `scripts/check.sh`)
...
- Modal shell **gate**: `role="dialog"` / `role="alertdialog"` and `aria-modal` may only be emitted by the shared shell — no such literal in `frontend/src/**/*.rs` except `frontend/src/app/focusable_menu.rs` (which owns `FocusableModalPanel` with `dialog_role` / `labelledby` / `testid` / `stop_pointerdown` / `on_escape`); the imperative equivalent (`set_attribute("role", "dialog")`) is caught too, passing `dialog_role="dialog"` to the shell is not; opt out in place with `modal-gate: allow <reason>` on the offending line (the image lightbox builds DOM imperatively and is the one current exemption); enforced by `scripts/check-modal-shell.sh` (hook + `scripts/check.sh`)
...
bash scripts/check-css-literals.sh       # per-file font-size / color / border-radius literal budget in frontend/styles must match the ratchet
bash scripts/check-token-mirrors.sh      # Android res + boot pages must mirror tokens.css :root (exact-equality, no allowlist)
bash scripts/check-icons.sh              # no literal <svg> outside icon.rs; svg rule sizes must be var(--icon-*)
bash scripts/check-modal-shell.sh        # role / aria-modal only emitted by app/focusable_menu.rs
