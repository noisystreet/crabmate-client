...
> **状态**：方案（已拍板走 **远程客户端**；**P3 已落地** `repl` 斜杠 `/help` `/workspace` `/conv`；**P4 已落地（M1–M4）**：全屏 `tui` 流式 transcript + 状态行 + 单行/多行输入 + Ctrl+C 取消 + 左栏会话与 `/status` 联动 + 审批浮层 + thinking 折叠 + 工具行摘要 + `/find` 搜索 + 翻页滚动 + 顶栏工作区 + 助手正文行内 Markdown 轻渲染 + 工作区目录树浏览（右栏仿 Desktop 默认显示；Ctrl+W 聚焦，含未设置时按 p 从项目池选择））  
