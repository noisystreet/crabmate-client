...
2. Tauri 打开连接页（默认预填最近一次成功连接的地址，含本机端口；无历史时用 `CM_DESKTOP_SUGGESTED_URL` 或默认 `http://127.0.0.1:8080/`）
...
| `frontend/` | 业务 UI（Leptos CSR）；契约 crates.io `crabmate` 0.5.2 + `protocol` |
