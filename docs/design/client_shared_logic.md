...
> **状态**：S1 **已落地**（`crates/crabmate-client-api`：`url` / `auth` / `secrets`）；S2+ 未开工  
...
```text
crates/
  crabmate-client-api/   # S1 已建：纯逻辑（url / auth / secrets；无 Tauri、无 web-sys、无 reqwest）
...
frontend/                # wasm fetch 适配器 + UI；S1 已用 client-api 规范化/鉴权头/槽名
```
...
**建议开工顺序**：S0 → S1（已完成）→ S2；S3/S4 可与终端钥匙串、续传功能并行。
...
| 下一步 | 实现 **S2**（`approval` 类型/解析；tui-core + frontend） |
