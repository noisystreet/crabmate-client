...
```text
crates/
  crabmate-client-api/   # 新建：纯逻辑（serde / alloc；无 Tauri、无 web-sys、无 reqwest）
  crabmate-connect/      # 保持：Tauri commands + keyring IO + CORS + handoff
  crabmate-tui-core/     # 变薄：reqwest ServeClient 调用 client-api
  crabmate-tui/          # CLI / TTY / slash 宿主
frontend/                # wasm fetch 适配器 + UI；调用 client-api 纯函数
```
...
| **S1** | `crabmate-client-api`：`url` + `auth` + `secrets` 常量；tui-core（及可选 connect）改依赖 | clippy；行为不变的表征测试 |
...
| **S5**（可选） | health 子集；斜杠名字表；frontend 全面改用 client-api | WASM 体积与编译时间可接受 |

**建议开工顺序**：S0 → S1 → S2；S3/S4 可与终端钥匙串、续传功能并行。
...
| 下一步 | 实现 **S1**（crate 脚手架 + URL/auth/secrets） |
