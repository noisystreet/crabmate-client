...
| Save file to this device | yes | reduced | yes | no | Desktop: native save dialog. Android: system share sheet. Web: browser download. Bytes via `GET /workspace/file/download` (PDF/binary). `GET /workspace/file/raw` is chat images only. Open IDE tab uses the buffer as UTF-8. Needs current `serve`. |
