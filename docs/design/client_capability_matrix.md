...
| Capability | Desktop | Android | Web | TUI | Notes |
|------------|---------|---------|-----|-----|-------|
| Settings panel editing serve user-data files | yes | yes | yes | reduced | Full-screen `tui` `/settings` / F2: edit **model name / API Base / temperature (`0.0..=2.0`) / thinking mode (server/on/off) / Agent role / session mode** in a partition panel; rows show three layers (local override `*` > serve user-data > `/status` defaults; blank = follow server). **API key** is edited in the same panel but only goes to the local keyring slot (see the auth Notes above; `--no-keyring` rejects the write). S saves with GET→merge→PUT into the same `/user-data/llm-overrides` (`client_llm.{model,api_base,temperature,llm_thinking_mode}`) and `/user-data/prefs` (`cm_role`/`session_mode`) the Desktop/Web settings use, preserving non-managed keys (`executor_llm` / `saved_models` / locale / theme / …), so values survive restarts and are shared across shells. Persisted temperature is sent per turn as top-level `temperature`, thinking only as `client_llm.llm_thinking_mode=on|off`. A running turn opens the panel read-only; Esc/F2 closes (dirty edits confirm first). Still W2: context tokens / tool-TTL switch / gateway presets. |
