...
| Stop in-flight turn (`POST …/cancel`) | yes | yes | yes | no | WASM Stop aborts SSE **and** cancel; abort-only left the server running (`stream_resume`). Needs Server **#900**. TUI does not call cancel yet. |
