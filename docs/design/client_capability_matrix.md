...
| Model `client_llm` key on device | yes | yes | reduced | reduced | Chat sends `client_llm.{api_key,model,api_base}` over HTTPS; do not `PUT /user-data/secrets/client-llm` from UI. TUI: `--llm-api-key` / `--llm-model` / `--llm-api-base` (`CM_API_KEY` / `CM_MODEL` / `CM_API_BASE`); missing API key falls back to the shell `client_llm` keyring slot, and the full-screen settings panel can **write/clear the same slot** (`com.crabmate.credentials` / `tauri_client_llm_api_key`) — `--no-keyring` disables both the fallback read and the panel write. |
...
| Capability | Desktop | Android | Web | TUI | Notes |
|------------|---------|---------|-----|-----|-------|
| Full-screen TUI (ratatui) | no | no | no | yes | `crabmate-tui tui`: top bar (workspace) + status bar + session sidebar + streaming transcript + composer. M1: stream + Ctrl+C cancel; M2: session sidebar with use/new + `/status` serve-default linkage; M3: approval overlay (Esc=deny), thinking fold (Ctrl+E), one-line tool summaries, `/find` search, paged scroll, multi-line input (Alt+Enter), a top bar showing the current workspace (via `GET /workspace`), and inline Markdown styling for assistant text (bold/italic/code/link/strike; code fences stay plain); M4: **workspace directory tree browse** in the **right-hand column, shown by default on wide terminals** (Desktop-style; Ctrl+W focuses it for ↑/↓ navigation, expand/collapse directories on demand via `GET /workspace[?path=]`, `w` returns to the sessions list). When the serve workspace is not set (empty root path) the right sidebar shows an explicit "not set" hint. `p` in the workspace tree opens the **project pool picker** in both cases (`GET /workspace/projects`; `POST /workspace/projects` switches and refreshes the tree — TUI only, no new serve contract): with no root it picks a first project, with one already set it switches to another serve project. Sidebar columns hidden below 120 columns; switching sessions starts a fresh transcript (no history replay). |
...
| Capability | Desktop | Android | Web | TUI | Notes |
|------------|---------|---------|-----|-----|-------|
| Settings panel editing serve user-data files | yes | yes | yes | reduced | Full-screen `tui` `/settings` / F2: edit **model name / API Base / temperature (`0.0..=2.0`) / context tokens (`0..=10_000_000`) / thinking mode (server/on/off) / Agent role / session mode / readonly tool TTL cache switch** in a partition panel; rows show three layers (local override `*` > serve user-data > `/status` defaults; blank = follow server). **API Base** edits through the same **gateway presets** as Web (`server` / ollama / deepseek / minimax / zhipu / moonshot / custom URL — same table as `client_llm_presets.rs`, cross-crate sync notes on both copies; TUI only writes `api_base`). **API key** is edited in the same panel but only goes to the local keyring slot (see the auth Notes above; `--no-keyring` rejects the write). S saves with GET→merge→PUT into the same `/user-data/llm-overrides` (`client_llm.{model,api_base,temperature,llm_context_tokens,llm_thinking_mode}`) and `/user-data/prefs` (`cm_role`/`session_mode`/`disable_readonly_tool_ttl_cache`) the Desktop/Web settings use, preserving non-managed keys (`executor_llm` / `saved_models` / locale / theme / …), so values survive restarts and are shared across shells. Per turn: top-level `temperature`; `client_llm.llm_thinking_mode=on|off`; `client_llm.llm_context_tokens` as a number (only when > 0); top-level `readonly_tool_ttl_cache_secs: 0` when the cache switch is off. A running turn opens the panel read-only; Esc/F2 closes (dirty edits confirm first). |
