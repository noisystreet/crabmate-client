...
> **单包 crates.io**：Server [`crates_io_single_package.md`](https://github.com/noisystreet/CrabMate/blob/main/docs/design/crates_io_single_package.md) — 钉 crates.io **`crabmate` `0.5.2`** + `features = ["protocol"]`。
...
```toml
crabmate = { version = "0.5.2", default-features = false, features = ["protocol"] }
```
...
crates.io **`crabmate` 0.5.2** 为默认渠道。未升级的 Client 仍可钉 crates.io / git **`v0.4.0` / `v0.5.0` / `v0.5.1`**；更旧版本可继续使用产品 tag **`v0.3.0`** / `client-contract-v0.2.0` 的旧多包名。
