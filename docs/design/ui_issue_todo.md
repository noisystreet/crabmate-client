...
- [ ] `frontend/styles` 消费侧仍有颜色字面量 20 处未 token 化（逐文件预算见 `scripts/css_literals_budget.txt`，现值为棘轮上限，新增即 fail，目标逐文件清到 0）。已修（2026-09-25，**Phase 1b**）：GFM alert 五色 `#3b82f6 / #22c55e / #a855f7 / #f59e0b / #ef4444` 改 `--info / --success / --accent / --warn / --error`（`color-mix(… 70%, var(--border))` 保饱和），`layout-chat.css` 内联错误底 `rgba(207,34,46,.08)` 改 `var(--error-bg)`，并删除 `--danger / --danger-bg / --ok` 兼容别名（`tokens.css` + 三主题，6 处消费点改指 `--error`）。剩余：`components.css` 6 处（深底 inset 高光 + 主按钮白字，待高光 / 前景 token）、`layout-chat.css` 4 处（3 处 `#fff` inset 高光 + 1 处浮层阴影）、`shell-ds.css` 3 / `shell-topbar.css` 2 / `mobile.css` 1 / `modal.css` 1 / `sidebar.css` 1（遮罩与浮层阴影，待阴影 token）、`tauri-shell.css` 2（壳层关闭键红底白字，壳层专用，可长期留预算）。
