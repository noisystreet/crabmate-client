...
- [ ] `frontend/styles` 消费侧仍有颜色字面量 26 处未 token 化（逐文件预算见 `scripts/css_literals_budget.txt`，现值为棘轮上限，新增即 fail，目标逐文件清到 0）。优先颜色：`layout-chat.css` alert 五色（`#3b82f6 / #22c55e / #a855f7 / #f59e0b / #ef4444`）待语义 token 化；`components.css` 深底 inset 高光与主按钮白字（待高光 / 前景 token）；`tauri-shell.css` 壳层关闭键红底白字（壳层专用，可长期留预算）。
