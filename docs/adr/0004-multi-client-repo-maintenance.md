...
本仓是**多端薄壳仓**：Desktop Tauri、Android Tauri、Web（`crabmate-web` 回环托管 Leptos CSR）、远程 `crabmate-tui` 四种交付形态，由 8 个 Rust 包组成（`desktop-tauri/src-tauri`、`mobile-tauri/src-tauri`、`crates/{crabmate-client-api,crabmate-tool-card,crabmate-connect,crabmate-tui}`、`web-host`、`frontend`；原 `crabmate-tui-core` 已并入 `crabmate-tui` `src/serve/`）。
