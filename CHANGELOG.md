...
- Workspace file tree: right-click a file → **Save to this device** (Desktop native save dialog; browser download; Android system share sheet). Uses the open IDE buffer if that path is already open; otherwise `GET /workspace/file`. Hidden for folders. Text files only (same limit as the IDE reader).
