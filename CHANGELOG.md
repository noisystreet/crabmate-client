...
- **Thinking echo dedupe**: when a model repeats its own reasoning inside the answer as a `### 思考过程` / `Thinking` section (with a `---` separator), the display layer strips that section so the thinking is shown once (in the collapsible block). Only affects display; stored text is unchanged.
