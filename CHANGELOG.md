...
- Chat composer image attachments (`POST /upload`) appear in the **user bubble** after send (same `/uploads/` URLs as the composer preview). Files live in the server temp uploads dir and may expire; they are not stored as bytes in the session.
