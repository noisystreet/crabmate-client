...
- Chat **Stop** now **`POST /chat/stream/{job_id}/cancel`** before aborting the SSE fetch, so the server stops the LLM turn and that turn’s background `run_command` jobs. Aborting the stream alone left the agent running (needed for `stream_resume`). Requires a `serve` that implements the cancel route.
