...
async fn send_message(client: &mut victauri_test::VictauriClient, text: &str) {
    ...
    let _ = client
        .eval_js("document.querySelector('[data-testid=\"chat-send-button\"]')?.click()")
        .await;
}
...
async fn wait_for_stream_end(client: &mut victauri_test::VictauriClient) {
    ...
    loop {
        ...
        if Instant::now() > deadline {
            panic!(
                "stream did not finish within {}s",
                REAL_LLM_TIMEOUT_MS / 1000
            );
        }
...
// ---------------------------------------------------------------------------
e2e_test!(
    real_llm_skills_prompt_reply_no_errors,
    |client| async move {
        ...
        assert!(
            rows > 1.0,
            "expected at least 2 chat-tui-turn (user + assistant), got {rows}"
        );
...
e2e_test!(real_llm_compile_hpcg_turn_layout, |client| async move {
    ...
    let assistant_count: f64 = client
        .eval_js("document.querySelectorAll('.msg-meta-role').length")
        .await
        .unwrap()
        .as_f64()
        .unwrap_or(0.0);
