...
// ---------------------------------------------------------------------------
e2e_test!(
    command_approval_opens_modal_with_preview,
    |client| async move {
        let sse = concat!(
...
// ---------------------------------------------------------------------------
e2e_test!(
    clarification_panel_submit_triggers_second_stream,
    |client| async move {
        ...
        Locator::test_id("composer-clarification-panel")
            .expect(&mut client)
            .to_be_visible()
            .await
            .unwrap();
        ...
        Locator::test_id("composer-clarification-submit")
            .click(&mut client)
            .await
            .unwrap();
        client
            .wait_for("text", Some("E2E after clarify"), Some(15000), Some(200))
            .await
            .unwrap();
        Locator::test_id("composer-clarification-panel")
            .expect(&mut client)
