...
工作区侧栏（或 IDE 文件树）可把**本机文件**拖到文件夹上上传（先确认；支持文本与二进制）。走 **`PUT /workspace/file/raw`**，需要当前 Server 构建（不是 crates.io / git **`v0.4.0`** 的 `serve`）。目标已存在时会再问是否覆盖；取消覆盖则**中止本批剩余文件**。聊天输入框附图（`POST /upload`）发送后会显示在用户气泡里；文件在服务端临时目录，可能被清理，会话里不存图片字节。
